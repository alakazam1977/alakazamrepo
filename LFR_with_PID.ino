...
int initial_motor_speed =80;        //can change the initial speed that you prefer
int maxSpeed = 70;                 //if the motor is too fast, decrease the max speed and vice versa
float Kp = 5;       //initialize to 1, reduce - overshoot and lose line. increase - cant navigate a turn
float Kd = 0;  //initialize to 1, increase until lesser wobbling
float Ki = 0;   //initialize to 0.01, jerk left or right if too high value.
float I=0, P=0, D=0, PD_value=0;
...
void setup()
{
  ...
  digitalWrite(LeftDir, HIGH);      //according to your connection from motor to the motor driver
}
